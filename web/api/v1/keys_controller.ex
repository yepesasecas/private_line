defmodule PrivateLine.V1.KeysController do
  ...
  def create(conn, param) do
    ...
    %{"token" => token,
      ...
      "destination_variables" => destination_variables} = param

    {:ok, decrypted_stone} = RsaEx.decrypt(stone, private_key)
    stone_msg = Poison.decode!(decrypted_stone)
    # stone_msg = %{first_name: "हिंद",
    #                         last_name: "Português",
    #                             email: "Pусский@gmail.com"}
    destination_response = Enum.reduce(destination_variables, destination_format, fn(var, acc) ->
      case Map.fetch(stone_msg, var) do
        {:ok, res} ->
          String.replace(acc, "{{{#{var}}}}", res)
        :error ->
          String.replace(acc, "{{{#{var}}}}", "")
      end
    end)

    response = %{destination_response: destination_response}
